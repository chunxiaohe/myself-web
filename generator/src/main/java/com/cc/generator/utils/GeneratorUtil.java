...
public class GeneratorUtil {
     ...
     * @return
     */
    public static String generateEntityProperties(List<ColumnInfo> infos, Integer type) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < infos.size(); i++) {
            String columnName = infos.get(i).getColumnName();
            //过滤掉公共实体中的字段
            if (type == 1) {
                if ("create_date".equals(columnName) || "create_by".equals(columnName) || "update_date".equals(columnName) || "update_by".equals(columnName)) {
                    continue;
                }
            }
            if (i != 0) {
            ...
            if (!StringUtils.isEmpty(infos.get(i).getAnnotation())) {
                sb.append("/**\n");
                sb.append("     ");
                String comment = "* " + infos.get(i).getAnnotation() + "\n";
                sb.append(comment);
                sb.append("    ");
                sb.append(" */\n");
                sb.append("    ");
     ...
     * @return 拼接的更新参数字符串
     */
    public static String generateUpdateParam(List<ColumnInfo> infos) {
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < infos.size(); i++) {
            if (i != 0) {
                builder.append("          ");
            }
            if (Objects.equals("Date", infos.get(i).getJavaType())) {
                builder.append("<if test = \" ").append(infos.get(i).getPropertyName()).append(" != null \" > ").append("\n");
            } else {
                builder.append("<if test = \" ").append(infos.get(i).getPropertyName()).append(" != null and ").append(infos.get(i).getPropertyName()).append(" != '' \">\n");
            }
            builder.append("            ");
            builder.append(infos.get(i).getColumnName()).append(" = #{").append(infos.get(i).getPropertyName()).append("},\n");
            builder.append("          </if>\n");
        }
    ...
    public static String generateEntityProperties(String parentClassName, List<ColumnInfo> infos) {
        StringBuilder sb = new StringBuilder(generateEntityProperties(infos, 1));
    ...
    public static String generateMapperColumnMap(String tableName, List<ColumnInfo> infos) {
        ...
        for (int i = 0; i < infos.size(); i++) {
            ...
            String baseSql;
            if (i == infos.size() - 1) {
                baseSql = infos.get(i).getColumnName();
            } else {
                baseSql = infos.get(i).getColumnName() + ",";
            }
            sb.append(baseSql);
    ...
    public static String generateMapperResultMap(List<ColumnInfo> infos) {
        ...
        for (ColumnInfo info : infos) {
            if (info.isPrimaryKey()) {
                ...
                sb.append("        ").append("<result column=\"").append(info.getColumnName()).append("\" property=\"").append(info.getPropertyName()).append("\"/> \n");
            }
        }
        return sb.toString();
    }
